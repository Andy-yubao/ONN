...
> 方法学边界：本探索实验在训练过程中逐轮查看 MNIST test set，并据此保存 best
> model / 判断 early stop。因此 91.27% 是 architecture exploration result，不能
> 作为严格独立 held-out test benchmark。
...
`results/training_curve.png`。训练时会在本地生成 `results/best_model.pt`；checkpoint
