"""Train matched IF/LIF variants; test is optional after validation selection."""
...
from .model import DeviceLIFConvSmall, normalized_temporal_weights
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eval-test", action="store_true",
                        help="Use only for a selected final configuration")
...
def main() -> None:
    ...
    run_config = {
        ...
        "decay": args.decay, "neuron": "IF" if args.decay == 1.0 else "LIF",
    ...
    if not args.eval_test:
        validation_accuracy, validation_seconds, workload = evaluate(
            model, validation_loader, encoder, device, collect_stats=True
        )
        ...
        metrics = {
            ...
            "seed": args.seed,
            "decay": args.decay,
            "neuron": "IF" if args.decay == 1.0 else "LIF",
            ...
            "workload_validation_per_sample": {
                key: value / validation_samples for key, value in workload.items()
    ...
    metrics = {
        ...
        "network": ("Conv1x16-LIF-subtract-Conv16x32(stride2)-LIF-subtract-FC512x10-final-membrane"
                    if args.readout_mode == "final_membrane"
                    else "Conv1x16-LIF-subtract-Conv16x32(stride2)-LIF-subtract-FC512x10-time-weighted-membrane"),
        "time_steps": encoder.time_steps,
