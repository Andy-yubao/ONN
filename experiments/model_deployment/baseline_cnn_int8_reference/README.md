...
## 9. 诊断验收（vs fake-quant 对照，阈值未放宽）——历史结果
...
> **99.95% 是针对 fake-quant 对照的诊断阈值。由于纯整数模型采用不同的 GAP 量化网格，该指标不再作为部署阻塞条件。** 部署验收按 §10 的独立门判定。

## 10. 部署验收结论（独立验收门）

对纯整数模型自身的部署资格单独判定，不依赖 fake-quant 对照一致率：

| 部署验收依据 | 结果 |
|---|---|
| 整数准确率与 FP32 相差不超过 0.2pp | **PASS**（0.01pp） |
| GAP 前所有可比整数节点逐位一致 | **PASS**（input_q→conv3_q 全部 100.0000%） |
| 所有累加器无溢出 | **PASS**（overflow=0） |
| 无回绕 | **PASS** |
| dtype 和范围符合协议 | **PASS** |
| 重复运行逐位一致 | **PASS**（两次运行 digest 相同） |
| GAP 差异来自已经冻结的设计语义 | **PASS**（首个差异节点 = `gap_q`） |
| **deployment_readiness** | **PASS** |

即：整数参考模型满足全部硬件安全标准（INT32、无溢出、无回绕、确定性、dtype/range），与 FP32 的准确率差 0.01pp，与 fake-quant 的唯一差异定位在冻结的整数 GAP 语义上，可作为部署候选。

## 11. 是否满足进入参数导出阶段的条件

**部署验收门：满足**（deployment_readiness = PASS，见 §10）。**fake-quant 诊断门：历史结果为 FAIL**（§9），该 99.95% 一致率是 fake-quant 对照的诊断指标，因纯整数模型采用不同的 GAP 量化网格而不再作为部署阻塞条件。

仍保留的决策点（仅当要求与 fake-quant 完全对齐时）：

1. **接受当前整数参考模型**（部署验收 PASS，推荐）：GAP 整数÷49（conv3 单位）是规范强制语义，模型满足全部硬件安全标准（INT32、无溢出、无回绕、确定性、dtype/range），且比 fake-quant 更接近 FP32（整数 vs FP32 = 99.81%），可直接进入参数导出。
